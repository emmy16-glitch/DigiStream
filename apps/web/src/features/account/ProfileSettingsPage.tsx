...
export function ResetPasswordPage() {
  const token = new URLSearchParams(window.location.search).get('token') ?? ''; const [password, setPassword] = useState(''); const [confirm, setConfirm] = useState(''); const [busy, setBusy] = useState(false); const [done, setDone] = useState(false); const [error, setError] = useState('');
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (password !== confirm) { setError('The passwords do not match.'); return; } setBusy(true); setError(''); try { await apiRequest('/api/v1/auth/password-reset/confirm', { method: 'POST', body: jsonBody({ token, password }) }); setDone(true); } catch (cause) { setError(message(cause)); } finally { setBusy(false); } }
  return <main className="account-page auth-recovery"><header className="account-header"><LinkButton href="/" variant="ghost">DigiStream</LinkButton><LinkButton href="/login" variant="ghost">Sign in</LinkButton></header><form className="invitation-card" onSubmit={submit}><span className="account-avatar">↯</span><h1>{done ? 'Password reset' : 'Reset your password'}</h1><p>{done ? 'Your password has been changed. Sign in with your new password.' : 'Choose a strong new password.'}</p>{done ? <LinkButton href="/login" variant="primary">Sign in</LinkButton> : <><label className="account-fields"><span>New password</span><input autoComplete="new-password" minLength={12} onChange={(event) => setPassword(event.target.value)} required type="password" value={password}/></label><label className="account-fields"><span>Confirm password</span><input autoComplete="new-password" minLength={12} onChange={(event) => setConfirm(event.target.value)} required type="password" value={confirm}/></label>{error ? <p className="account-error" role="alert">{error}</p> : null}<Button disabled={!token} loading={busy} type="submit" variant="primary">Reset password</Button></>}</form></main>;
}
