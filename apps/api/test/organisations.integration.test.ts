...
test(
  'organisation routes enforce broadcaster creation and tenant permissions',
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(ownerList.json().organisations.length, 1);
      assert.equal(ownerList.json().organisations[0].role, 'owner');
