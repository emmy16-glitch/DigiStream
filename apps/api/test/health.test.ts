...
test('GET /api/v1/status declares chat, creator, listener, backstage and realtime capabilities', async () => {
  ...
  try {
    ...
    assert.equal(response.statusCode, 200);
    assert.equal(response.json().stage, 'notification-inbox');
    assert.ok(response.json().capabilities.includes('durable-notification-inbox'));
    assert.ok(
      response.json().capabilities.includes('notification-read-and-archive-state'),
    );
    assert.ok(
      response.json().capabilities.includes('notification-realtime-delivery-preference'),
    );
