...
| UIQ-007 | P1 | Cross-product responsive audit | Existing responsive smoke tests did not exercise the complete agreed 1440×900, 1280×720, 768×1024, 390×844, 360×800 and 844×390 matrix. Entry and onboarding slices found real clipping, guidance and focus defects; Creator-core audit then found sub-44px creator brand/workspace controls and a 32px Studio diagnostics disclosure target. | In progress — slice 3 | Slices 1 and 2 are merged with successful exact-current-main CI. Slice 3 raises Creator shell and Studio interaction targets and adds an exact six-viewport browser matrix across shell, Overview, Broadcasts and Studio using real authorised organisation/channel/broadcast state. Slice 3 still requires PR CI, review, merge and exact-current-main push CI. |
...
- **Slice 3 — Creator shell, Overview, Broadcasts and Studio:** in progress. The Creator home brand link and workspace selector now meet the 44px interaction floor at desktop/tablet/short-landscape widths; Studio diagnostics disclosure controls are raised from 32px to 44px. The dedicated matrix creates real authorised multi-workspace, active-channel and draft-broadcast state, then checks shell controls, Overview actions, Broadcasts rows/tabs and the Studio dialog at all six viewports without joining LiveKit or fabricating media readiness.
